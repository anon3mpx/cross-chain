...
- Addressed in the current hardening pass: `V2`, `V7`, `V9`, `V10`, `A2`, `A4`, `A8`, `A9`, `U1`, `U2`, `U6`, `U8`, `U10`.
- Partially addressed in the current hardening pass: `A2` keeps query-string auth as a compatibility fallback while SDK/server now prefer `Sec-WebSocket-Protocol`.
- Still open from the prioritized set: `V1`, `V3`, `V5`, `A3`, `A10`, `U3`, `U4`, `U7`, and the additional paymaster signature-construction finding.
...
| V2 | Confirmed | `RecoveryEngine.start()` schedules `_runCycle()` on `setInterval` without a running/reentrancy guard (`src/vps/services/RecoveryEngine.ts:24-30`). If a cycle takes longer than the interval, overlapping recovery cycles can process the same stale list. Update (2026-06-04): addressed by adding a process-local in-flight guard in `src/vps/services/RecoveryEngine.ts`. |
...
| V9 | Confirmed, with stronger local evidence | `PaymasterService` records cache entries by `token` (`src/vps/services/PaymasterService.ts:120-135`) but reads them by `${chainId}:${token}` (`:157-162`), so the cache usually misses. The stored timestamp is never checked. The current behavior is stale/missing rate protection plus fallback pricing, not just timestamp staleness. Update (2026-06-04): addressed by normalizing cache keys and enforcing a bounded cache age in `src/vps/services/PaymasterService.ts`. |
| V10 | Confirmed | `_computeOfferSet()` uses nested `Promise.all` across route/provider quote builders (`src/vps/services/QuoteEngine.ts:767-786`). One provider rejection can fail the entire offer-set computation instead of degrading that provider only. Update (2026-06-04): addressed by isolating per-route quote failures so they degrade to `null` rather than aborting the whole offer-set build in `src/vps/services/QuoteEngine.ts`. |
...
| A2 | Confirmed | Current SDK sends the API key in the WebSocket query string (`src/vps/sdk/EmpxCrossChainSDK.ts:123-125`), and the server reads `?key=` (`src/vps/api/WebSocketAPI.ts:37-45`). This leaks through URL logs/proxies/history. Prefer `Sec-WebSocket-Protocol` or a short-lived signed WS token. Update (2026-06-04): partially addressed by moving the SDK/server preferred auth path to `Sec-WebSocket-Protocol`; query-string auth remains as a compatibility fallback in `src/vps/sdk/EmpxCrossChainSDK.ts` and `src/vps/api/WebSocketAPI.ts`. |
...
| A4 | Confirmed | Same root as `V7`: signed status actions have timestamp freshness but no nonce/idempotency record (`src/vps/api/StatusAPI.ts:649-697`). Update (2026-06-04): addressed by nonce-bearing message support and replay-store enforcement in `src/vps/api/StatusAPI.ts`. |
...
| A8 | Confirmed | Rate-limit keys trust `cf-connecting-ip` and `x-forwarded-for` directly (`src/vps/api/StatusAPI.ts:152-161`). Unless Express is behind a trusted proxy configuration that strips/spoofs these headers, clients can evade per-IP limits. Update (2026-06-04): addressed by defaulting to socket/Express IPs and only honoring proxy headers behind an explicit env toggle in `src/vps/api/StatusAPI.ts`. |
| A9 | Confirmed with precondition | Redis quote cache stores encoded quote JSON without MAC/signature (`src/vps/cache/QuoteCache.ts:45-62`). This is exploitable if Redis is writable by an attacker or a compromised internal component. It is not internet-reachable by default in compose (`config/docker/docker-compose.yml:21-27`). Update (2026-06-04): addressed by adding optional HMAC signing/verification for Redis-backed quote-cache entries in `src/vps/cache/QuoteCache.ts`. |
...
| ID | Disposition | Validation |
| --- | --- | --- |
| U1 | Confirmed | DRPC public key is hardcoded in browser config (`/Users/ganadhish/code/work/EMPSEAL-UI/src/config/rpc.ts:1-15`). This is not a secret once shipped to browsers, but it enables quota abuse and should be restricted by domain/provider policy or proxied. Update (2026-06-04): addressed by removing the hardcoded key and deriving DRPC URLs only from `VITE_DRPC_PUBLIC_KEY` when present in `EMPSEAL-UI/src/config/rpc.ts`. |
| U2 | Confirmed | CoinGecko keys are read from `VITE_*` env variables and sent from browser fetches (`/Users/ganadhish/code/work/EMPSEAL-UI/src/lib/api/coingecko.ts:48-56`). Vite exposes these to the client bundle. The batching claim is partly stale: token-price requests batch 100 addresses (`:123-145`), not unbatched single calls. Update (2026-06-04): addressed by making browser-side key attachment opt-in behind `VITE_ENABLE_BROWSER_API_KEYS === "true"` in `EMPSEAL-UI/src/lib/api/coingecko.ts`. |
...
| U6 | Confirmed for legacy swap modal | `ConnectWallet` renders wallet rows without `onClick` handlers (`/Users/ganadhish/code/work/EMPSEAL-UI/src/pages/swap/ConnectWallet.jsx:35-58`). This is a broken UX path. Other pages may use RainbowKit/wagmi instead, so this is scoped to this modal. Update (2026-06-04): addressed by wiring the modal rows to actual wagmi connectors in `EMPSEAL-UI/src/pages/swap/ConnectWallet.jsx`. |
...
| U8 | Confirmed | `vercel.json` only defines a rewrite and no security headers (`/Users/ganadhish/code/work/EMPSEAL-UI/vercel.json:1-3`). Add CSP, frame, content-type, referrer, and permissions-policy headers. Update (2026-06-04): addressed by adding CSP, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, and `Permissions-Policy` headers in `EMPSEAL-UI/vercel.json`. |
...
| U10 | Confirmed, wording adjusted | There are 240 frame files totaling about 6.0 MB under `public/frames`. `FrameCanvas` eagerly creates all 240 `Image()` objects on mount (`/Users/ganadhish/code/work/EMPSEAL-UI/src/pages/landing/FrameCanvas.tsx:19-21`, `:123-135`). These are not part of the JS "initial bundle", but they are eagerly requested when the landing component mounts. Update (2026-06-04): addressed by switching to windowed frame preloading around the active frame in `EMPSEAL-UI/src/pages/landing/FrameCanvas.tsx`. |
...
3. Done 2026-06-04: add nonce/idempotency to signed status actions and store consumed action ids.
4. Partially done 2026-06-04: fix `PaymasterService` rate cache key/TTL. Still open: verify paymaster signature packing against the deployed Solidity paymaster.
5. Partially done 2026-06-04: make quote building provider-isolated. Still open: per-provider timeouts and circuit breakers.
6. Partially done 2026-06-04: move WS API auth off URL query strings for the preferred SDK/server path and restrict trusted proxy handling. Still open: tighten CORS and remove the compatibility fallback when safe.
7. Partially done 2026-06-04: add Vercel security headers and stop implicit browser CoinGecko key attachment; DRPC now requires env configuration. Still open: move paid RPC/API traffic behind a backend proxy or provider-side domain restrictions.
8. Done 2026-06-04: lazy-load landing frames and fix the legacy swap `ConnectWallet` modal handlers.
...
- `npx tsx --test tests/vps/recovery-engine.test.ts tests/vps/status-api-signed-actions.test.ts tests/vps/quote-cache.test.ts tests/vps/paymaster-service.test.ts tests/vps/websocket-api.test.ts tests/vps/quote-engine-provider-isolation.test.ts` in `/Users/ganadhish/code/work/ruflo`: passed, 8/8 tests.
- `npm test -- --run src/config/rpc.test.ts src/lib/api/coingecko.test.ts` in `/Users/ganadhish/code/work/EMPSEAL-UI`: passed, 4/4 tests.
- `npm run build` in `/Users/ganadhish/code/work/EMPSEAL-UI`: passed. Vite still emitted non-fatal sourcemap/comment warnings and large chunk warnings.
